...
- (void)setFieldWithModel: (LLTextFieldModel *)model {
    ...
    if ((model.rightViewText.length > 0)) {
...
- (void)configRightView {
    ...
    [btn setTitle:self.model.rightViewText forState:UIControlStateNormal];
    ...
    if (self.model.rightViewAction.length > 0 && self.target) {
        [btn addTarget:self.target action:NSSelectorFromString(self.model.rightViewAction) forControlEvents:UIControlEventTouchUpInside];
    }
    self.rightView = btn;
